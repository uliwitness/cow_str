...
namespace cow {
	...
	/*! str guarantees:
		...
		Note that str is not a template class, as it is assumed that every sensible
		app these days uses UTF-8, and it's therefore unnecessary to provide additional
		character widths.
	 */
	class str {
	public:
	...
	public:
		...
		void insert(iterator dest, size_t count, char ch);
		void insert(iterator dest, const_iterator beg, const_iterator ed);
